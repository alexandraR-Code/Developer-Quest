...
// Número único de referencia de un certificado, generado una sola vez y
// reutilizado después (persiste en localStorage). Compartido por los 3
// certificados: cada uno pasa su propia clave de storage y prefijo.
function obtenerNumeroCertificado(claveStorage, prefijo) {
  let numero = localStorage.getItem(claveStorage);
  if (!numero) {
    const sufijo = Math.floor(10000 + Math.random() * 90000);
    numero = `${prefijo}${new Date().getFullYear()}-${sufijo}`;
    localStorage.setItem(claveStorage, numero);
  }
  return numero;
}
...
// Diseño institucional compartido entre el certificado de cierre total
// del programa y el certificado de la Fase 1; solo cambian el número,
// el texto del cuerpo, el subtítulo y el sufijo del archivo.
function generarCertificadoBase(numeroCertificado, textoCuerpo, textoSubtitulo, sufijoArchivo) {
  ...
  doc.setTextColor(...gris);
  const cuerpo = doc.splitTextToSize(textoCuerpo, 170);
  ...
  doc.setFontSize(12.5);
  doc.setTextColor(...moradoOscuro);
  doc.text(textoSubtitulo, centroX, 146, { align: "center" });
  ...
  doc.save(`TeamCoderExperience_${sufijoArchivo}_${nombreJugador.replace(/\s+/g, "_")}.pdf`);
}

function generarCertificadoPDF() {
  generarCertificadoBase(
    obtenerNumeroCertificado("dq_certificado_numero", "TCE-"),
    "Por su dedicación, esfuerzo y constancia a lo largo de todo el recorrido, culminando con éxito la totalidad del programa:",
    `¡Finalizamos! · Niveles 1 al ${niveles.length} completados`,
    "Certificado"
  );
}
